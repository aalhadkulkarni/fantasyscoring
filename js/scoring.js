...
function addDescriptions(playerId, descriptions) {
    if (playerId == undefined || !scoringDetails[playerId]) {
        return;
    }
...
function addInnings(iplInnings) {
    ...
    for (let i = 0; i < bowlingCard.length; i++) {
        ...
        if (playerId == null) {
            console.log("Could not find " + iplBowlerStats.PlayerName + ", skipping the player");
            continue;
        }
        bowlingStats.push({
            "playerId": playerId,
...
function getIntlMatchData(matchId) {
    ...
    $.ajax({
        url: "https://api.icc.cdp.pulselive.com/fixtures/" + matchId + "/scoring",
        dataType: 'jsonp',
    });
}
